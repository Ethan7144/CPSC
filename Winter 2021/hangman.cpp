...
bool Hangman::newWord()
{
    ...
    if (wins + losses == totalWords)
        return false;
    while(!reset){
        newWord = rand() % totalWords;
        if (!list[newWord].used){
            currentWord = newWord;
            list[newWord].used = true;
            reset = true;
        }
        // above code sets the new word up
        // newWord is the index of the word
    }
    for(int i = 0; i < BODY_SIZE; i++){
        body[i].displaySecond = false;
        body[i].displayThird = false;
    ...
    // newWord resets everything basically
    return true;
}
