...
which will store ONLY Unique numbers in the array. Any non unique number will
be ignored and not used in the array when it is attempted to be implemented.
Anytime this happens, the state for the object will change, which affects the
any, target, and sum functions of the dataExtractors class. Each state will
only be changed the number is not unique upon being entered. If the state has never been
...
If the array is empty, none of the functions will work besides the addXArray and
addYArray. The Client will not be allowed to access any of the variables inside the class,
...
Keep in mind: The Array needs to have something in it, or else a ArgumentException will be
thrown and the client session will end. This class is also not able to access the child
classes.
