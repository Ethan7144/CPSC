...
pthread_mutex_t data2_mutex = PTHREAD_MUTEX_INITIALIZER;
...
void* takeOrders(void* arg)
{
    ...
    for(int i = 0; i < MAX; ++i) {
        ...
        // Get user input
        if (num_new_orders == 0)
            pthread_cond_wait(&not_empty, &data2_mutex);
        ...
        pthread_mutex_unlock(&data2_mutex);
...
void* processOrders(void* arg)
{
    ...
    for(int i = 0; i < MAX; ++i) {
        ...
        pthread_mutex_lock(&data2_mutex);
        if(num_new_orders== 0)
            pthread_cond_wait(&not_full, &data2_mutex);
